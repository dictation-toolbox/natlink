...
for add on packages such as Unimacro, Vocola2, and Dragonfly2.
...
Release 5.1 (August 1, 2021): should work reasonable for the UserDirectory users, so Dragonfly users could try this release. Note this release is done from the makeflitinstall branch.

For Unimacro and Vocola(2), please wait a little bit longer, they will be fixed ASAP.

Note a new repository has been added for Speech Engine independent actions, like accessing the clipboard and performing actions like sendkeys to the foreground window: this is the work in progress `dtactions`, which will be automatically installed with `natlink`. Also `dragonfly2` is installed together with `natlink`.

You can ask for help in getting in working on the [KnowBrainer Forums](https://www.knowbrainer.com/forums/forum/categories.cfm?catid=25&entercat=y&CFTREEITEMKEY=25) forums if you have difficulty in getting Natlink in combination with Dragonfly working. 
