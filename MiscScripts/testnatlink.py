...
#
# TODO: when using NatSpeak version 5 we do not see exceptions when a
# recognition mimic fails !!!!
...
#---------------------------------------------------------------------------
# This switches to NatSpeak.  The way to switch has changed in Version 5
# of NatSpeak.

versionFive = 0  # Version 5 support does not work for this script.
...
def testDictObj():
    ...
    if versionFive:
        ...
        callTest.testTextChange(moduleInfo,(5,10,'another phrase',19,19))
    ...
    testForException(natlink.MimicFailed,"natlink.recognitionMimic(['Select','hello'])")
    testFuncReturn((37,50),"dictObj.getTextSel()",locals())
    ...
    testForException(natlink.MimicFailed,"natlink.recognitionMimic(['Select','this','is'])")
    testFuncReturn((37,50),"dictObj.getTextSel()",locals())
    callTest.testTextChange(moduleInfo,None)

    testForException(natlink.MimicFailed,"natlink.recognitionMimic(['Select','all','done'])")
    testFuncReturn((37,50),"dictObj.getTextSel()",locals())
    callTest.testTextChange(moduleInfo,None)
...
def testWordFuncs():
    ...
    testFuncReturn(dgnwordflag_nodelete+dgnwordflag_title_mode,"natlink.getWordInfo('and')")
    
    testFuncReturn(None,"natlink.getWordInfo('FrotzBlatz',0)")
    testFuncReturn(None,"natlink.getWordInfo('FrotzBlatz',7)")
    
    testFuncReturn(None,"natlink.getWordInfo('Szymanski',0)")
    testFuncReturn(None,"natlink.getWordInfo('Szymanski',6)")
    testFuncReturn(0,"natlink.getWordInfo('Szymanski',1)")
    testFuncReturn(None,"natlink.getWordInfo('szymanski',1)")
    testFuncReturn(0,"natlink.getWordInfo('Szymanski',5)")
    ...
    testForException(natlink.UnknownName,"natlink.setWordInfo('Szymanski',0)")
    ...
    testFuncReturn(None,"natlink.getWordInfo('Szymanski',0)")
    testFuncReturn(0,"natlink.getWordInfo('Szymanski',1)")
    testFuncReturn(1,"natlink.addWord('Szymanski',dgnwordflag_is_period)")
    testFuncReturn(dgnwordflag_is_period,"natlink.getWordInfo('Szymanski')")
    ...
    testForException(natlink.UnknownName,"natlink.deleteWord('Szymanski')")
...
def testNatLinkMain():
    ...
    try:
        ...
        testRecognition(['jMg','1'],0)
        createMacroFile(baseDirectory,'__jMg1.py','1')

        try:
            testRecognition(['jMg','1'],0)
        except TestError:
            # This failure will happen if testnatlink is run twice in a row
            print 
            print '*******'
            print 'One of the NatLink tests has failed.'
            print
            print 'This particular failure usually means that you ran testnatlink twice'
            print 'in a row without shutting down Dragon NaturallySpeaking.'
            print
            print 'To fix this:'
            print '(1) Shutdown Python'
            print '(2) Shutdown Dragon NaturallySpeaking'
            print '(3) Try this selftest again'
            raise ExitQuietly
            
        toggleMicrophone()

        try:
            testRecognition(['jMg','1'],1)
        except MimicFailed:
            # This failure will happen if NatLink is not enabled
            print
            print '*******'
            print 'One of the NatLink tests has failed.'
            print
            print 'This particular failure usually means that NatLink has not been'
            print 'enabled for the currently running version of Dragon NaturallySpeaking.'
            print
            print 'To fix this:'
            print '(1) Shutdown Python'
            print '(2) Shutdown Dragon NaturallySpeaking'
            print '(3) Run c:\NatLink\MacroSystem\EnableNL.exe'
            print '(4) Try this selftest again'
            raise ExitQuietly
        ...
        testRecognition(['jMg','5'],0)
        natlink.execScript('AppBringUp "calc"')
        testRecognition(['jMg','5'],1)
        natlink.playString('{Alt+F4}')
    ...
    # from them
    testRecognition(['jMg','1'],0)
    testRecognition(['jMg','2'],0)
    testRecognition(['jMg','3'],0)
    testRecognition(['jMg','4'],0)
    testRecognition(['jMg','5'],0)
...
def testWordProns():
    ...
    testFuncReturn(None,"natlink.getWordProns('Szymanski')")

    # I have looked up the expected pronunciations for these words
    testFuncReturn(['and','~'],"natlink.getWordProns('and')")
