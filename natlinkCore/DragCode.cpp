...
BOOL CDragonCode::setMicState( const char * pState )
{
	...
	if( 0 == stricmp( pState, "on" ) )
	{
		wState = DGNMIC_ON;
	}
	else
	if( 0 == stricmp( pState, "off" ) )
	{
		wState = DGNMIC_OFF;
	}
	else
	if( 0 == stricmp( pState, "sleeping" ) )
	{
...
BOOL CDragonCode::startTraining( char * pMode )
{
	...
	DWORD dwMode;
	if( 0 == stricmp( pMode, "calibrate" ) )
	{
		dwMode = DGNTRNMODE_CALIBRATE;
	}
	else
	if( 0 == stricmp( pMode, "shorttrain" ) )
	{
		dwMode = DGNTRNMODE_SHORTBATCH;
	}
	else
	if( 0 == stricmp( pMode, "longtrain" ) )
	{
		dwMode = DGNTRNMODE_CROSSWORD;
	}
	else
	if( 0 == stricmp( pMode, "batchadapt" ) )
	{
